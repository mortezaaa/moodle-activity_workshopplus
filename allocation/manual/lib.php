 ...
 */
class workshopplus_manual_allocator implements workshopplus_allocator {
    ...
    public function ui() {
        ...
        $output     = $PAGE->get_renderer('workshopplusallocation_manual');

        $page       = optional_param('page', 0, PARAM_INT);
        $perpage    = get_user_preferences('workshopplusallocation_manual_perpage', 10);
        $groupid    = groups_get_activity_group($this->workshopplus->cm, true);
        ...
        if ($m) {
            ...
            case self::MSG_ADDED:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new workshopplus_message(get_string('allocationadded', 'workshopplusallocation_manual'),
                    workshopplus_message::TYPE_OK);
                break;
            case self::MSG_EXISTS:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new workshopplus_message(get_string('allocationexists', 'workshopplusallocation_manual'),
                    workshopplus_message::TYPE_INFO);
                break;
            case self::MSG_NOSUBMISSION:
                $hlauthorid     = $m[1];
                $message        = new workshopplus_message(get_string('nosubmissionfound', 'workshopplus'),
                    workshopplus_message::TYPE_ERROR);
                break;
            case self::MSG_CONFIRM_DEL:
                $hlauthorid     = $m[2];
                $hlreviewerid   = $m[3];
                if ($m[4] == 0) {
                    $message    = new workshopplus_message(get_string('areyousuretodeallocate', 'workshopplusallocation_manual'),
                        workshopplus_message::TYPE_INFO);
                } else {
                    $message    = new workshopplus_message(get_string('areyousuretodeallocategraded', 'workshopplusallocation_manual'),
                        workshopplus_message::TYPE_ERROR);
                }
                $url = new moodle_url($PAGE->url, array('mode' => 'del', 'what' => $m[1], 'confirm' => 1, 'sesskey' => sesskey()));
                $label = get_string('iamsure', 'workshopplus');
                $message->set_action($url, $label);
                break;
            case self::MSG_DELETED:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new workshopplus_message(get_string('assessmentdeleted', 'workshopplus'),
                    workshopplus_message::TYPE_OK);
                break;
            case self::MSG_DELETE_ERROR:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new workshopplus_message(get_string('assessmentnotdeleted', 'workshopplus'),
                    workshopplus_message::TYPE_ERROR);
                break;
            }
        }

        // fetch the list of ids of all workshopplus participants
        $numofparticipants = $this->workshopplus->count_participants(false, $groupid);
        $participants = $this->workshopplus->get_participants(false, $groupid, $perpage * $page, $perpage);
        ...
        if ($hlauthorid > 0 and $hlreviewerid > 0) {
            ...
            $button = $output->single_button($PAGE->url, get_string('showallparticipants', 'workshopplusallocation_manual'), 'get');
        } else {
            $button = '';
        }
        ...
        $submissions = $this->workshopplus->get_submissions(array_keys($participants));
        ...
        if ($participants) {
            ...
            $sql = "SELECT a.id AS assessmentid, a.submissionid,
                     ...
                     WHERE u.id $participantids AND s.workshopplusid = :workshopplusid AND s.example = 0";
        ...
        $data                   = new workshopplusallocation_manual_allocations();
        $data->workshopplus         = $this->workshopplus;
        $data->allocations      = $allocations;
        $data->userinfo         = $userinfo;
        $data->authors          = $this->workshopplus->get_potential_authors();
        $data->reviewers        = $this->workshopplus->get_potential_reviewers();
        $data->hlauthorid       = $hlauthorid;
        $data->hlreviewerid     = $hlreviewerid;
        $data->selfassessment   = $this->workshopplus->useselfassessment;

        // prepare the group selector
        $groupselector = $output->container(groups_print_activity_menu($this->workshopplus->cm, $PAGE->url, true), 'groupwidget');
 ...
 *
 * @see workshopplus_manual_allocator::ui()
 */
class workshopplusallocation_manual_allocations implements renderable {

    /** @var workshopplus module instance */
    public $workshopplus;
