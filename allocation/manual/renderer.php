 ...
 */
class workshopplusallocation_manual_renderer extends mod_workshopplus_renderer  {
     ...
     *
     * @param workshopplusallocation_manual_allocations $data to be displayed
     * @return string html code
     */
    protected function render_workshopplusallocation_manual_allocations(workshopplusallocation_manual_allocations $data) {

        $this->workshopplus     = $data->workshopplus;
        ...
        $selfassessment     = $data->selfassessment;    // bool is the self-assessment allowed in this workshopplus?
        ...
        $table->head        = array(get_string('participantreviewedby', 'workshopplus'),
                                    get_string('participant', 'workshopplus'),
                                    get_string('participantrevierof', 'workshopplus'));
    ...
    /**
     * Returns information about the workshopplus participant
     *
     * @return string HTML code
     */
    protected function helper_participant(stdclass $allocation, array $userinfo) {
        ...
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nosubmissionfound', 'workshopplus'), 'info');
        } else {
            $link = $this->workshopplus->submission_url($allocation->submissionid);
            ...
            if (is_null($allocation->submissiongrade)) {
                $o .= $this->output->container(get_string('nogradeyet', 'workshopplus'), array('grade', 'missing'));
            } else {
                $o .= $this->output->container(get_string('alreadygraded', 'workshopplus'), array('grade', 'missing'));
            }
        }
    ...
    protected function helper_reviewers_of_participant(stdclass $allocation, array $userinfo, array $reviewers, $selfassessment) {
        $o = '';
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nothingtoreview', 'workshopplus'), 'info');
        } else {
            $exclude = array();
            if (! $selfassessment) {
            ...
            if ($options) {
                ...
                $select = new single_select($handler, 'by', $options, '', array(''=>get_string('chooseuser', 'workshopplus')), 'addreviewof' . $allocation->userid);
                $select->set_label(get_string('addreviewer', 'workshopplusallocation_manual'));
    ...
    protected function helper_reviewees_of_participant(stdclass $allocation, array $userinfo, array $authors, $selfassessment) {
        $o = '';
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('withoutsubmission', 'workshopplus'), 'info');
        }
        $exclude = array();
        if (! $selfassessment) {
            $exclude[$allocation->userid] = true;
            $o .= $this->output->container(get_string('selfassessmentdisabled', 'workshopplus'), 'info');
        }
        // todo add an option to exclude users without own submission
        ...
        if ($options) {
            ...
            $select = new single_select($handler, 'of', $options, '', array(''=>get_string('chooseuser', 'workshopplus')), 'addreviewby' . $allocation->userid);
            $select->set_label(get_string('addreviewee', 'workshopplusallocation_manual'));
            $o .= $this->output->render($select);
        } else {
            $o .= $this->output->container(get_string('nothingtoreview', 'workshopplus'), 'info');
        }
