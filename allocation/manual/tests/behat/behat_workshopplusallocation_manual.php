...
/**
 * Steps definitions related to workshopplusallocation_manual.
 *
 * @package    workshopplusallocation_manual
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_workshopplusallocation_manual extends behat_base {
    /**
     * Manually adds a reviewer for workshopplus participant.
     *
     * This step should start on manual allocation page.
     *
     * @When /^I add a reviewer "(?P<reviewer_name_string>(?:[^"]|\\")*)" for workshopplus participant "(?P<participant_name_string>(?:[^"]|\\")*)"$/
     * @param string $reviewername
     * @param string $participantname
     */
    public function i_add_a_reviewer_for_workshopplus_participant($reviewername, $participantname) {
        ...
        try {
            ...
            $this->find_button(get_string('showallparticipants', 'workshopplusallocation_manual'))->press();
            $selectnode = $this->find('xpath', $xpathselect);
        }
        ...
        if (!$this->running_javascript()) {
            ...
            $allocatedtext = $this->getSession()->getSelectorsHandler()->xpathLiteral(
                    get_string('allocationadded', 'workshopplusallocation_manual'));
            $this->find('xpath', "//*[contains(.,$allocatedtext)]");
        }
    }

    /**
     * Manually allocates multiple reviewers in workshopplus.
     *
     * @When /^I allocate submissions in workshopplus "(?P<workshopplus_name_string>(?:[^"]|\\")*)" as:"$/
     ...
     */
    public function i_allocate_submissions_in_workshopplus_as($workshopplusname, TableNode $table) {

        $this->find_link($workshopplusname)->click();
        $this->find_link(get_string('allocate', 'workshopplus'))->click();
        $rows = $table->getRows();
        ...
        for ($i = 1; $i < count($rows); $i++) {
            $this->i_add_a_reviewer_for_workshopplus_participant($rows[$i][$reviewer], $rows[$i][$participant]);
        }
    }
}
