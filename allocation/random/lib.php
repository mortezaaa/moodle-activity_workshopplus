 ...
 *
 * @package    workshopplusallocation_random
 ...
 * Allocates the submissions randomly
 */
class workshopplus_random_allocator implements workshopplus_allocator {
     ...
     * @param workshopplus_allocation_result allocation result logger
     */
    public function execute(workshopplus_random_allocator_setting $settings, workshopplus_allocation_result $result) {

        //$myfile = fopen("//newfile.txt", "w") or die(print_r());       
        
        $authors        = $this->workshopplus->get_potential_authors();
        $authors        = $this->workshopplus->get_grouped($authors);
        $reviewers      = $this->workshopplus->get_potential_reviewers(!$settings->assesswosubmission);
        $reviewers      = $this->workshopplus->get_grouped($reviewers);
        $assessments    = $this->workshopplus->get_all_assessments();
        
        //by Moreteza
        $hiwis          = $this->workshopplus->get_hiwis();
        //end by Morteza
        ...
        if ($settings->numofreviews) {
            ...
            $result->log(get_string('numofrandomlyallocatedsubmissions', 'workshopplusallocation_random', count($randomallocations)));
        ...
        if ($settings->addselfassessment) {
            ...
            $result->log(get_string('numofselfallocatedsubmissions', 'workshopplusallocation_random', count($selfallocations)));
            unset($selfallocations);
        }
        
        ///////by Morteza
        //// Allocations of all submissions to hiwis Hiwis
        $hiwiassess = array();
        $submissioncount = count($authors[0]);
        $submissionkey = array_keys($authors[0]);        
        $hiwiscount = count($hiwis);
        $hiwikey = array_keys($hiwis);
                        
        $submissionrandomidx = range(0, $submissioncount-1);
        shuffle($submissionrandomidx);        
        
        $hiwiiterator = 0;
        for ($i=0; $i<$submissioncount; $i++){
            $authorindex    = $submissionkey[$submissionrandomidx[$i]];
            $authorid       = $authors[0][$authorindex]->id;
            $hiwiid         = $hiwis[$hiwikey[$hiwiiterator]]->id;
            $hiwiassess[]   = array($hiwiid => $authorid);
            $hiwiiterator   = ($hiwiiterator+1)%$hiwiscount; 
        }
        
        // Check whether the check box for TAs has been set    
        // If yes, allocations for TAs is to be done from the student-TA mapping table
        if($settings->defaulttaallocation){
            $hiwiassess = workshopplus_student_ta_map($this->workshopplus->id,$this->course->id);
        }
        
        $newallocations = array_merge($newallocations, $hiwiassess);
       ///
       
        if (empty($newallocations)) {
            $result->log(get_string('noallocationtoadd', 'workshopplusallocation_random'), 'info');
        } else {
            ...
            foreach ($newallocations as $newallocation) {
                ...
                if (isset($allreviewers[$reviewerid])) {
                    ...
                    if (!$allreviewersreloaded) {
                        $allreviewers = $this->workshopplus->get_potential_reviewers(false);
                ...
                if (in_array($newallocation, $newnonexistingallocations)) {
                    $result->log(get_string('allocationaddeddetail', 'workshopplusallocation_random', $a), 'ok', 1);
                } else {
                    $result->log(get_string('allocationreuseddetail', 'workshopplusallocation_random', $a), 'ok', 1);
                }
            }
        }
        if ($settings->removecurrent) {
            ...
            // by reviewer
            $result->log(get_string('numofdeallocatedassessment', 'workshopplusallocation_random', count($delassessments)), 'info');
            foreach ($delassessments as $delassessmentkey => $delassessmentid) {
                ...
                if (!is_null($assessments[$delassessmentid]->grade)) {
                    $result->log(get_string('allocationdeallocategraded', 'workshopplusallocation_random', $a), 'error', 1);
                    unset($delassessments[$delassessmentkey]);
                } else {
                    $result->log(get_string('assessmentdeleteddetail', 'workshopplusallocation_random', $a), 'info', 1);
                }
            }
            $this->workshopplus->delete_assessment($delassessments);
        }
        $result->set_status(workshopplus_allocation_result::STATUS_EXECUTED);
    }
    ...
    public function ui() {
        ...
        $message = new workshopplus_message();
        if ($m == self::MSG_SUCCESS) {
            $message->set_text(get_string('randomallocationdone', 'workshopplusallocation_random'));
            $message->set_type(workshopplus_message::TYPE_OK);
        }
        ...
        if (VISIBLEGROUPS == $gmode or SEPARATEGROUPS == $gmode) {
            $users = $this->workshopplus->get_potential_authors() + $this->workshopplus->get_potential_reviewers();
            $users = $this->workshopplus->get_grouped($users);
            if (isset($users[0])) {
                ...
                if (!empty($nogroupusers)) {
                    ...
                    $a = implode(', ', $list);
                    $out .= $output->box(get_string('nogroupusers', 'workshopplusallocation_random', $a), 'generalbox warning nogroupusers');
                }
            }
        }

        // TODO $out .= $output->heading(get_string('stats', 'workshopplusallocation_random'));
     ...
     *
     * @param array $authors grouped of {@see workshopplus::get_potential_authors()}
     * @param array $reviewers grouped by {@see workshopplus::get_potential_reviewers()}
     * @param array $assessments as returned by {@see workshopplus::get_all_assessments()}
    ...
    protected function self_allocation($authors=array(), $reviewers=array(), $assessments=array()) {
        ...
        ////////////////////////////
        ///////////by Morteza
        $reviewerlist = $reviewers[0];
        unset($reviewers[0]);
        foreach ($reviewers as $groupid => $groupmembers) {
            // for all authors in all groups
            $currentgroupauthorid = 0;
            foreach ($groupmembers as $memberid => $member){ // two loops. the first one is to find the author in the group. the second to assign 
                // self assessment inside the group to that author.
                if (isset($authors[0][$memberid])) { // if the reviewer is already an author
                    $currentgroupauthorid = $memberid;
                }   
            }  
            foreach ($groupmembers as $memberid => $member){ // two loops. the first one is to find the author in the group. the second to assign
                // self assessment inside the group to that author.
                if ($currentgroupauthorid and !isset($alreadyallocated[$memberid])) {
                    // and the allocation does not exist yet, then
                    $add[] = array($memberid => $currentgroupauthorid);
                }                
            }          
        }
	//print_r($add);
...
//         }
    ...
    protected function add_new_allocations(array $newallocations, array $dataauthors, array $datareviewers) {
        ...
        $submissions    = $this->workshopplus->get_submissions($authorids);
        $submissions    = $this->index_submissions_by_authors($submissions);
        foreach ($newallocations as $newallocation) {
            list($reviewerid, $authorid) = each($newallocation);
            if (!isset($submissions[$authorid])) {
                throw new moodle_exception('unabletoallocateauthorwithoutsubmission', 'workshopplus');
            }
            $submission = $submissions[$authorid];
            $status = $this->workshopplus->add_allocation($submission, $reviewerid, 1, true);   // todo configurable weight?
            if (workshopplus::ALLOCATION_EXISTS == $status) {
        ...
        if (is_array($submissions)) {
            foreach ($submissions as $submissionid => $submission) {
                if (isset($byauthor[$submission->authorid])) {
                    throw new moodle_exception('moresubmissionsbyauthor', 'workshopplus');
                }
     ...
     *
     * @param array $assessments array of assessments as returned by {@link workshopplus::get_all_assessments()}
...
class workshopplus_random_allocator_setting {
    ...
    /** @var bool random allocation for student reviewers and TA allocation from table {workshopplus_stu_ta_map} */
    public $defaulttaallocation;
    ...
    public static function instance_from_object(stdClass $data) {
        ...
        foreach (array('excludesamegroup', 'defaulttaallocation', 'removecurrent', 'assesswosubmission', 'addselfassessment') as $k) {
            if (isset($data->$k)) {
                $i->$k = (bool)$data->$k;
            } else {
                $i->$k = false;
