 ...
 * The capability 'mod/workshopplus:peerassess' is intentionally not checked here.
...
if ($assessmenteditable and $workshopplus->useexamples and $workshopplus->examplesmode == workshopplus::EXAMPLES_BEFORE_ASSESSMENT
...
//        'weight' => $assessment->weight,
    ...
    if ($assessmenteditable and $workshopplus->overallfeedbackmode) {
        $currentdata = file_prepare_standard_editor($currentdata, 'feedbackauthor', $workshopplus->overall_feedback_content_options(),
            $workshopplus->context, 'mod_workshopplus', 'overallfeedback_content', $assessment->id);
        if ($workshopplus->overallfeedbackfiles) {
            $currentdata = file_prepare_standard_filemanager($currentdata, 'feedbackauthorattachment',
                $workshopplus->overall_feedback_attachment_options(), $workshopplus->context, 'mod_workshopplus', 'overallfeedback_attachment',
                $assessment->id);
        }
    }
    ...
    if ($mform->is_cancelled()) {
        ...
        if (isset($data->feedbackauthor_editor)) {
            ...
            $coredata = file_postupdate_standard_editor($coredata, 'feedbackauthor', $workshopplus->overall_feedback_content_options(),
                $workshopplus->context, 'mod_workshopplus', 'overallfeedback_content', $assessment->id);
        ...
        if (!is_null($rawgrade) and isset($data->saveandclose)) {
            ...
            $next = reset($pending);
            if (!empty($next)) {
                redirect($workshopplus->assess_url($next->id));
            } else {
...
if ($canoverridegrades or $cansetassessmentweight) {
    ...
    $feedbackform = $workshopplus->get_feedbackreviewer_form($PAGE->url, $assessment, $options);
    if ($data = $feedbackform->get_data()) {
        $data = file_postupdate_standard_editor($data, 'feedbackreviewer', array(), $workshopplus->context);
        $record = new stdclass();
        ...
        if ($canoverridegrades) {
            $record->gradinggradeover = $workshopplus->raw_grade_value($data->gradinggradeover, $workshopplus->gradinggrade);
...
// output starts here
$output = $PAGE->get_renderer('mod_workshopplus');      // workshopplus renderer
echo $output->header();
echo $output->heading(format_string($workshopplus->name));
echo $output->heading(get_string('assessedsubmission', 'workshopplus'), 3);

$submission = $workshopplus->get_submission_by_id($submission->id);     // reload so can be passed to the renderer
