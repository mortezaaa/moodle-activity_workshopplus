...
/**
 * Defines the complete workshopplus structure for backup, with file and id annotations
 *
 * @link http://docs.moodle.org/dev/workshopplus for XML structure diagram
 */
class backup_workshopplus_activity_structure_step extends backup_activity_structure_step {

    /**
     * Defines the structure of the 'workshopplus' element inside the workshopplus.xml file
     *
    ...
    protected function define_structure() {
        ...
        $workshopplus = new backup_nested_element('workshopplus', array('id'), array(
        ...
        $this->add_subplugin_structure('workshopplusform', $workshopplus, true);

        // grading evaluations data
        $this->add_subplugin_structure('workshoppluseval', $workshopplus, true);
        ...
        $this->add_subplugin_structure('workshopplusform', $referenceassessment, true);
        ...
        $this->add_subplugin_structure('workshopplusform', $exampleassessment, true);
        ...
        $this->add_subplugin_structure('workshopplusform', $assessment, true);
        ...
        ////////////////////////////////////////////////////////////////////////
        $workshopplus->add_child($examplesubmissions);
        ...
        $workshopplus->set_source_table('workshopplus', array('id' => backup::VAR_ACTIVITYID));

        $examplesubmission->set_source_sql("
            SELECT *
              FROM {workshopplus_submissions}
             WHERE workshopplusid = ? AND example = 1",
        ...
        if ($userinfo) {
            ...
            $submission->set_source_sql("
                SELECT *
                  FROM {workshopplus_submissions}
                 WHERE workshopplusid = ? AND example = 0",
            ...
            $aggregation->set_source_table('workshopplus_aggregations', array('workshopplusid' => backup::VAR_PARENTID));
        }
        ...
        $workshopplus->annotate_files('mod_workshopplus', 'intro', null); // no itemid used
        $workshopplus->annotate_files('mod_workshopplus', 'instructauthors', null); // no itemid used
        $workshopplus->annotate_files('mod_workshopplus', 'instructreviewers', null); // no itemid used
        $workshopplus->annotate_files('mod_workshopplus', 'conclusion', null); // no itemid used
        ...
        // return the root element (workshopplus), wrapped into standard activity structure
        return $this->prepare_activity_structure($workshopplus);
    }
}
