...
/**
 * Keeps track of upgrades to the workshopplus module
 *
 ...
 *
 * workshopplus supports upgrades from version 1.9.0 and higher only. During 1.9 > 2.0 upgrade,
 * there are significant database changes.
 *
 * @param int $oldversion the version we are upgrading from
 * @return bool result
 */
function xmldb_workshopplus_upgrade($oldversion) {
    ...
    if ($oldversion < 2012033100) {
        // add the field 'phaseswitchassessment' to the 'workshopplus' table
        ...
        upgrade_mod_savepoint(true, 2012033100, 'workshopplus');
    }

    /**
     * Remove all workshopplus calendar events
     */
    if ($oldversion < 2012041700) {
        require_once($CFG->dirroot . '/calendar/lib.php');
        $events = $DB->get_records('event', array('modulename' => 'workshopplus'));
        foreach ($events as $event) {
            $event = calendar_event::load($event);
            $event->delete();
        }
        upgrade_mod_savepoint(true, 2012041700, 'workshopplus');
    }

    /**
     * Recreate all workshopplus calendar events
     */
    if ($oldversion < 2012041701) {
        ...
        foreach ($rs as $workshopplus) {
            $cmid = $workshopplus->cmid;
            unset($workshopplus->cmid);
            workshopplus_calendar_update($workshopplus, $cmid);
        }
        $rs->close();
        upgrade_mod_savepoint(true, 2012041701, 'workshopplus');
    }
    ...
    if ($oldversion < 2012102400) {
        ...
        upgrade_mod_savepoint(true, 2012102400, 'workshopplus');
    }
    ...
    /**
     * Add overall feedback related fields into the workshopplus table.
     */
    if ($oldversion < 2013032500) {
        ...
        upgrade_mod_savepoint(true, 2013032500, 'workshopplus');
    }

    /**
     * Add feedbackauthorattachment field into the workshopplus_assessments table.
     */
    if ($oldversion < 2013032501) {
        $table = new xmldb_table('workshopplus_assessments');
        $field = new xmldb_field('feedbackauthorattachment', XMLDB_TYPE_INTEGER, '3', null, null, null, '0', 'feedbackauthorformat');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_mod_savepoint(true, 2013032501, 'workshopplus');
    }
