 ...
 */
class workshopplus_best_evaluation extends workshopplus_evaluation {

    /** @var workshopplus the parent workshopplus instance */
    protected $workshopplus;

    /** @var the recently used settings in this workshopplus */
    protected $settings;

    /**
     * Constructor
     *
     * @param workshopplus $workshopplus The workshopplus api instance
     * @return void
     */
    public function __construct(workshopplus $workshopplus) {
        global $DB;
        $this->workshopplus = $workshopplus;
        $this->settings = $DB->get_record('workshoppluseval_best_settings', array('workshopplusid' => $this->workshopplus->id));
    }
    ...
    public function update_grading_grades(stdclass $settings, $restrict=null) {
        ...
        // Remember the recently used settings for this workshopplus.
        if (empty($this->settings)) {
            $record = new stdclass();
            $record->workshopplusid = $this->workshopplus->id;
            $record->comparison = $settings->comparison;
            $DB->insert_record('workshoppluseval_best_settings', $record);
        } elseif ($this->settings->comparison != $settings->comparison) {
            $DB->set_field('workshoppluseval_best_settings', 'comparison', $settings->comparison,
                    array('workshopplusid' => $this->workshopplus->id));
        }

        // Get the grading strategy instance.
        $grader = $this->workshopplus->grading_strategy_instance();
     ...
     *
     * @return workshopplus_best_evaluation_settings_form
     */
    public function get_settings_form(moodle_url $actionurl=null) {
        ...
        return new workshopplus_best_evaluation_settings_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public static function delete_instance($workshopplusid) {
        ...
        $DB->delete_records('workshoppluseval_best_settings', array('workshopplusid' => $workshopplusid));
    }
 ...
 */
class workshopplus_best_evaluation_settings_form extends workshopplus_evaluation_settings_form {
    ...
    protected function definition_sub() {
        ...
        $plugindefaults = get_config('workshoppluseval_best');
        $current = $this->_customdata['current'];

        $options = array();
        for ($i = 9; $i >= 1; $i = $i-2) {
            $options[$i] = get_string('comparisonlevel' . $i, 'workshoppluseval_best');
        }
        $mform->addElement('select', 'comparison', get_string('comparison', 'workshoppluseval_best'), $options);
        $mform->addHelpButton('comparison', 'comparison', 'workshoppluseval_best');
