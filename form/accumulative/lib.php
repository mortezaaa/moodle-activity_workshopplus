...
function workshopplusform_accumulative_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    ...
    if (!$dimension = $DB->get_record('workshopplusform_accu', array('id' => $itemid ,'workshopplusid' => $workshopplus->id))) {
        send_file_not_found();
    }
 ...
 */
class workshopplus_accumulative_strategy implements workshopplus_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'workshopplusform_accumulative', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new workshopplus_edit_accumulative_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->accumulative;  // records to be saved into {workshopplusform_accu}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id         = $DB->insert_record('workshopplusform_accu', $record);
            } else {
                // exiting field
                $DB->update_record('workshopplusform_accu', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $PAGE->context, 'workshopplusform_accumulative', 'description', $record->id);
            $DB->update_record('workshopplusform_accu', $record);
        }
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'workshopplusform_accumulative', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new workshopplus_accumulative_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public static function scale_used($scaleid, $workshopplusid=null) {
        ...
        if (!is_null($workshopplusid)) {
            $conditions['workshopplusid'] = $workshopplusid;
        }
        return $DB->record_exists('workshopplusform_accu', $conditions);
    }
    ...
    public static function delete_instance($workshopplusid) {
        ...
        $DB->delete_records('workshopplusform_accu', array('workshopplusid' => $workshopplusid));
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($PAGE->context->id, 'workshopplusform_accumulative', 'description', $id);
            }
        }
        $DB->delete_records_list('workshopplusform_accu', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link workshopplus_edit_accumulative_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->accumulative = array();        // records to be stored in {workshopplusform_accu}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->accumulative[$i]                     = new stdclass();
            $cook->accumulative[$i]->id                 = $raw->{'dimensionid__idx_'.$i};
            $cook->accumulative[$i]->workshopplusid         = $this->workshopplus->id;
     ...
     */
    public function update_peer_grade(stdclass $assessment) {
        $grades     = $this->get_current_assessment_data($assessment);
    ...
    protected function calculate_peer_grade(array $grades) {
        ...
        foreach ($grades as $grade) {
            ...
            if ($dimension->grade < 0) {
                ...
                // regular grade- Changed by Morteza, to have sum of partss instead of average of parts
		$sumgrades  += $grade->grade * $dimension->weight * 100;
                $sumweights += $dimension->grade*$dimension->weight;
            }
        }
