 ...
 * @uses moodleform
 */
class workshopplus_edit_numerrors_strategy_form extends workshopplus_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            ...
            $mform->addElement('hidden', 'dimensionid__idx_'.$i);   // the id in workshopplus_forms
            $mform->setType('dimensionid__idx_'.$i, PARAM_INT);
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                    get_string('dimensiondescription', 'workshopplusform_numerrors'), '', $descriptionopts);
            ...
            $mform->addElement('select', 'weight__idx_'.$i,
                    get_string('dimensionweight', 'workshopplusform_numerrors'), workshopplus::available_dimension_weights_list());
        ...
        $mform->addElement('static', 'mappinginfo', get_string('maperror', 'workshopplusform_numerrors'),
                                                            get_string('mapgrade', 'workshopplusform_numerrors'));
        ...
        for ($i = 1; $i <= $totalweight; $i++) {
            ...
            $selects[] = $mform->createElement('static', 'mapdefault__idx_'.$i, '',
                                        get_string('percents', 'workshopplusform_numerrors', floor(100 - $i * 100 / $totalweight)));
            $mform->addGroup($selects, 'grademapping'.$i, $i, array(' '), false);
            $mform->setDefault('map__idx_'.$i, '');
        }

        $mform->registerNoSubmitButton('noadddims');
        $mform->addElement('submit', 'noadddims', get_string('addmoredimensions', 'workshopplusform_numerrors',
                workshopplus_numerrors_strategy::ADDDIMS));
