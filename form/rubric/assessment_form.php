 ...
 */
abstract class workshopplus_rubric_assessment_form extends workshopplus_assessment_form {
    ...
    public function validation($data, $files) {
        ...
        for ($i = 0; isset($data['dimensionid__idx_'.$i]); $i++) {
            if (empty($data['chosenlevelid__idx_'.$i])) {
                $errors['chosenlevelid__idx_'.$i] = get_string('mustchooseone', 'workshopplusform_rubric'); // used in grid
                $errors['levelgrp__idx_'.$i] = get_string('mustchooseone', 'workshopplusform_rubric');      // used in list
            }
        }
        return $errors;
    }
 ...
 */
class workshopplus_rubric_list_assessment_form extends workshopplus_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $nodims; $i++) {
            // dimension header
            $dimtitle = get_string('dimensionnumber', 'workshopplusform_rubric', $i+1);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $levelgrp[] = $mform->createElement('radio', 'chosenlevelid__idx_'.$i, '',
                        format_text($definition, $definitionformat, null, $workshopplus->course->id), $levelid);
            }
 ...
 */
class workshopplus_rubric_grid_assessment_form extends workshopplus_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $numofcolumns = array_reduce($levelcounts, 'workshopplus::lcm', 1);

        $mform->addElement('header', 'rubric-grid-wrapper', get_string('layoutgrid', 'workshopplusform_rubric'));

        $mform->addElement('html', '<table class="rubric-grid">' . "\n");
        $mform->addElement('html', '<th class="header">' . get_string('criteria', 'workshopplusform_rubric') . '</th>');
        $mform->addElement('html', '<th class="header" colspan="'.$numofcolumns.'">'.get_string('levels', 'workshopplusform_rubric').'</th>');
