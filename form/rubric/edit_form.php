 ...
 * @uses moodleform
 */
class workshopplus_edit_rubric_strategy_form extends workshopplus_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'workshopplusform_rubric', $i+1));
            ...
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                                get_string('dimensiondescription', 'workshopplusform_rubric'), '', $descriptionopts);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $mform->addGroup($levelgrp, 'level__idx_'.$i.'__idy_'.$j, get_string('levelgroup', 'workshopplusform_rubric'), array(' '), false);
        ...
        $mform->addElement('submit', 'adddims', get_string('addmoredimensions', 'workshopplusform_rubric',
                workshopplus_rubric_strategy::ADDDIMS));
        $mform->closeHeaderBefore('adddims');

        $mform->addElement('header', 'configheader', get_string('configuration', 'workshopplusform_rubric'));
        $layoutgrp = array();
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutlist', 'workshopplusform_rubric'), 'list');
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutgrid', 'workshopplusform_rubric'), 'grid');
        $mform->addGroup($layoutgrp, 'layoutgrp', get_string('layout', 'workshopplusform_rubric'), array('<br />'), false);
