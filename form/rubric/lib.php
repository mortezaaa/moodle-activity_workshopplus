 ...
 * @return bool
 */
function workshopplusform_rubric_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('workshopplusform_rubric', array('id' => $itemid ,'workshopplusid' => $workshopplus->id))) {
        send_file_not_found();
    }
 ...
 */
class workshopplus_rubric_strategy implements workshopplus_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        $this->descriptionopts = array_merge(array('context' => $this->workshopplus->context), $this->descriptionopts);
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $this->workshopplus->context, 'workshopplusform_rubric', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new workshopplus_edit_rubric_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        if ($DB->record_exists('workshopplusform_rubric_config', array('workshopplusid' => $this->workshopplus->id))) {
            $DB->set_field('workshopplusform_rubric_config', 'layout', $layout, array('workshopplusid' => $this->workshopplus->id));
        } else {
            $record = new stdclass();
            $record->workshopplusid = $this->workshopplus->id;
            $record->layout     = $layout;
            $DB->insert_record('workshopplusform_rubric_config', $record, false);
        }
        ...
        foreach ($data as $record) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('workshopplusform_rubric', $record);
            } else {
                // exiting field
                $DB->update_record('workshopplusform_rubric', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $this->workshopplus->context, 'workshopplusform_rubric', 'description', $record->id);
            $DB->update_record('workshopplusform_rubric', $record);
            ...
            foreach ($record->levels as $level) {
                ...
                if (empty($level->id)) {
                    // new field
                    $level->id = $DB->insert_record('workshopplusform_rubric_levels', $level);
                } else {
                    // exiting field
                    $DB->update_record('workshopplusform_rubric_levels', $level);
                }
            }
        }
        $DB->delete_records_list('workshopplusform_rubric_levels', 'id', $deletelevs);
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $this->workshopplus->context->id, 'workshopplusform_rubric', 'description',
        ...
        $formclassname = 'workshopplus_rubric_' . $this->config->layout . '_assessment_form';
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id AS id, MIN(l.grade) AS min, MAX(l.grade) AS max, 1 AS weight
                  FROM {workshopplusform_rubric} d
            INNER JOIN {workshopplusform_rbrc_lvls} l ON (d.id = l.dimensionid)
                 WHERE d.workshopplusid = :workshopplusid
              GROUP BY d.id';
    ...
    public static function delete_instance($workshopplusid) {
        ...
        $dimensions = $DB->get_records('workshopplusform_rubric', array('workshopplusid' => $workshopplusid), '', 'id');
        $DB->delete_records_list('workshopplusform_rubric_levels', 'dimensionid', array_keys($dimensions));
        $DB->delete_records('workshopplusform_rubric', array('workshopplusid' => $workshopplusid));
        $DB->delete_records('workshopplusform_rubric_config', array('workshopplusid' => $workshopplusid));
    }
    ...
    protected function load_fields() {
        ...
        $sql = "SELECT r.id AS rid, r.sort, r.description, r.descriptionformat,
                  ...
                  FROM {workshopplusform_rubric} r
             LEFT JOIN {workshopplusform_rbrc_lvls} l ON (l.dimensionid = r.id)
                 WHERE r.workshopplusid = :workshopplusid
    ...
    protected function load_config() {
        ...
        if (!$config = $DB->get_record('workshopplusform_rubric_config', array('workshopplusid' => $this->workshopplus->id), 'layout')) {
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($this->workshopplus->context->id, 'workshopplusform_rubric', 'description', $id);
            }
        }
        $DB->delete_records_list('workshopplusform_rubric', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link workshopplus_edit_rubric_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        for ($i = 0; $i < $raw->norepeats; $i++) {
            ...
            $cook[$i]->workshopplusid           = $this->workshopplus->id;
            $cook[$i]->sort                 = $i + 1;
     ...
     * @return array [int dimensionid] => stdclass workshopplus_grades record
     */
