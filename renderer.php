...
/**
 * workshopplus module renderering methods are defined here
 *
 * @package    mod
...
/**
 * workshopplus module renderer class
 *
 * @copyright 2009 David Mudrak <david.mudrak@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_workshopplus_renderer extends plugin_renderer_base {

    ////////////////////////////////////////////////////////////////////////////
    // External API - methods to render workshopplus renderable components
    ////////////////////////////////////////////////////////////////////////////

    /**
     * Renders workshopplus message
     *
     * @param workshopplus_message $message to display
     * @return string html code
     */
    protected function render_workshopplus_message(workshopplus_message $message) {
        ...
        case workshopplus_message::TYPE_OK:
            $sty = 'ok';
            break;
        case workshopplus_message::TYPE_ERROR:
            $sty = 'error';
            break;
        default:
            $sty = 'info';
    ...
    /**
     * Renders full workshopplus submission
     *
     * @param workshopplus_submission $submission
     * @return string HTML
     */
    protected function render_workshopplus_submission(workshopplus_submission $submission) {
        global $CFG;
        ...
        if (!$anonymous) {
            ...
            $a->url             = $userurl->out();
            $byfullname         = get_string('byfullname', 'workshopplus', $a);
        ...
        $created = get_string('userdatecreated', 'workshopplus', userdate($submission->timecreated));
        $o .= $this->output->container($created, 'userdate created');

        if ($submission->timemodified > $submission->timecreated) {
            $modified = get_string('userdatemodified', 'workshopplus', userdate($submission->timemodified));
     ...
     *
     * @param workshopplus_submission_summary $summary
     * @return string text to be echo'ed
     */
    protected function render_workshopplus_submission_summary(workshopplus_submission_summary $summary) {
        ...
        if ($summary->status == 'notgraded') {
            $classes    .= ' notgraded';
            $gradestatus = $this->output->container(get_string('nogradeyet', 'workshopplus'), 'grade-status');

        } else if ($summary->status == 'graded') {
            $classes    .= ' graded';
            $gradestatus = $this->output->container(get_string('alreadygraded', 'workshopplus'), 'grade-status');
        }
        ...
        $created = get_string('userdatecreated', 'workshopplus', userdate($summary->timecreated));
        $o .= $this->output->container($created, 'userdate created');

        if ($summary->timemodified > $summary->timecreated) {
            $modified = get_string('userdatemodified', 'workshopplus', userdate($summary->timemodified));
            $o .= $this->output->container($modified, 'userdate modified');
        }
    ...
    /**
     * Renders full workshopplus example submission
     *
     * @param workshopplus_example_submission $example
     * @return string HTML
     */
    protected function render_workshopplus_example_submission(workshopplus_example_submission $example) {
        ...
        $content = file_rewrite_pluginfile_urls($example->content, 'pluginfile.php', $this->page->context->id,
                                                        'mod_workshopplus', 'submission_content', $example->id);
     ...
     *
     * @param workshopplus_example_submission_summary $summary
     * @return string text to be echo'ed
     */
    protected function render_workshopplus_example_submission_summary(workshopplus_example_submission_summary $summary) {
        ...
        // additional info
        if ($summary->status == 'notgraded') {
            $o .= $this->output->container(get_string('nogradeyet', 'workshopplus'), 'example-info nograde');
        } else {
            $o .= $this->output->container(get_string('gradeinfo', 'workshopplus' , $summary->gradeinfo), 'example-info grade');
        }
     ...
     *
     * @param workshopplus_user_plan $plan prepared for the user
     * @return string html code to be displayed
     */
    protected function render_workshopplus_user_plan(workshopplus_user_plan $plan) {
        ...
        foreach ($plan->phases as $phasecode => $phase) {
            ...
            foreach ($phase->actions as $action) {
                ...
                case 'switchphase':
                    $icon = 'i/marker';
                    if ($phasecode == workshopplus::PHASE_ASSESSMENT
                            and $plan->workshopplus->phase == workshopplus::PHASE_SUBMISSION
                            and $plan->workshopplus->phaseswitchassessment) {
                        $icon = 'i/scheduled';
                    }
                    $actions .= $this->output->action_icon($action->url, new pix_icon($icon, get_string('switchphase', 'workshopplus')));
                    break;
                }
            }
            if (!empty($actions)) {
    ...
    protected function render_workshopplus_grading_report(workshopplus_grading_report $gradingreport) {
        ...
        $table->head[] = $this->helper_sortable_heading(get_string('submission', 'workshopplus'), 'submissiontitle',
        ...
        if ($options->showsubmissiongrade) {
            $table->head[] = $this->helper_sortable_heading(get_string('submissiongradeof', 'workshopplus', $data->maxgrade),
        ...
        if ($options->showgradinggrade) {
            $table->head[] = $this->helper_sortable_heading(get_string('gradinggradeof', 'workshopplus', $data->maxgradinggrade),
            ...
            for ($tr = 0; $tr < $numoftrs; $tr++) {
                ...
                if ($tr % $spanreceived == 0) {
                    ...
                    $cell->text = $this->helper_grading_report_assessment($assessment, $options->showreviewernames, $userinfo,
                            get_string('gradereceivedfrom', 'workshopplus'));
                ...
                if ($tr % $spangiven == 0) {
                    ...
                    $cell->text = $this->helper_grading_report_assessment($assessment, $options->showauthornames, $userinfo,
                            get_string('gradegivento', 'workshopplus'));
     ...
     *
     * @param workshopplus_feedback_author $feedback
     * @return string HTML
     */
    protected function render_workshopplus_feedback_author(workshopplus_feedback_author $feedback) {
     ...
     *
     * @param workshopplus_feedback_reviewer $feedback
     * @return string HTML
     */
    protected function render_workshopplus_feedback_reviewer(workshopplus_feedback_reviewer $feedback) {
        return $this->helper_render_feedback($feedback);
    }
     ...
     *
     * @param workshopplus_feedback_author|workshopplus_feedback_reviewer $feedback
     * @return string HTML
     */
    private function helper_render_feedback($feedback) {
        ...
        $o .= $this->output->container_start('header');
        $o .= $this->output->heading(get_string('feedbackby', 'workshopplus', s(fullname($feedback->get_provider()))), 3, 'title');
     ...
     *
     * @param workshopplus_assessment $assessment
     * @return string HTML
     */
    protected function render_workshopplus_assessment(workshopplus_assessment $assessment) {
        ...
        if (!empty($assessment->title)) {
            ...
            $title = get_string('assessment', 'workshopplus');
        }
        ...
        if (!$anonymous) {
            ...
            $a->url     = $userurl->out();
            $byfullname = get_string('assessmentby', 'workshopplus', $a);
            $oo         = $this->output->container($userpic, 'picture');
            $oo        .= $this->output->container($byfullname, 'fullname');
        ...
        if (is_null($assessment->realgrade)) {
            $o .= $this->output->container(
                get_string('notassessed', 'workshopplus'),
                'grade nograde'
            );
        } else {
            $a              = new stdClass();
            ...
            $o .= $this->output->container(
                get_string('gradeinfo', 'workshopplus', $a),
                'grade'
            );

            if (!is_null($assessment->weight) and $assessment->weight != 1) {
                $o .= $this->output->container(
                    get_string('weightinfo', 'workshopplus', $assessment->weight),
                    'weight'
                );
            }
        }
        ...
        if (!is_null($assessment->form)) {
            $o .= print_collapsible_region_start('assessment-form-wrapper', uniqid('workshopplus-assessment'),
                    get_string('assessmentform', 'workshopplus'), '', false, true);
     ...
     *
     * @param workshopplus_example_assessment $assessment
     * @return string HTML
     */
    protected function render_workshopplus_example_assessment(workshopplus_example_assessment $assessment) {
        return $this->render_workshopplus_assessment($assessment);
    }

    /**
     * Renders the reference assessment of an example submission
     *
     * @param workshopplus_example_reference_assessment $assessment
     * @return string HTML
     */
    protected function render_workshopplus_example_reference_assessment(workshopplus_example_reference_assessment $assessment) {
        return $this->render_workshopplus_assessment($assessment);
    }
     ...
     * @return string HTML
     */
    protected function overall_feedback(workshopplus_assessment $assessment) {
        ...
        $o = print_collapsible_region($o, 'overall-feedback-wrapper', uniqid('workshopplus-overall-feedback'),
            get_string('overallfeedback', 'workshopplus'), '', false, true);

        return $o;
    }

    /**
     * Renders a perpage selector for workshopplus listings
     *
    ...
    public function perpage_selector($current=10) {
        ...
        $select = new single_select($this->page->url, 'perpage', $options, '', array('' => get_string('showingperpagechange', 'mod_workshopplus')));
        $select->label = get_string('showingperpage', 'mod_workshopplus', $current);
        $select->method = 'post';
     ...
     *
     * @param workshopplus_final_grades $grades with the info about grades in the gradebook
     * @return string HTML
     */
    protected function render_workshopplus_final_grades(workshopplus_final_grades $grades) {
        ...
        if (!empty($grades->submissiongrade)) {
            ...
            $out .= html_writer::tag(
                'div',
                html_writer::tag('div', get_string('submissiongrade', 'mod_workshopplus'), array('class' => 'gradetype')) .
        ...
        if (!empty($grades->assessmentgrade)) {
            ...
            $out .= html_writer::tag(
                'div',
                html_writer::tag('div', get_string('gradinggrade', 'mod_workshopplus'), array('class' => 'gradetype')) .
    ...
    protected function helper_submission_attachments($submissionid, $format = 'html') {
        ...
        $files  = $fs->get_area_files($ctx->id, 'mod_workshopplus', 'submission_attachment', $submissionid);
        ...
        foreach ($files as $file) {
            ...
            $fileurl    = moodle_url::make_pluginfile_url($ctx->id, 'mod_workshopplus', 'submission_attachment',
                            $submissionid, $filepath, $filename, true);
            $embedurl   = moodle_url::make_pluginfile_url($ctx->id, 'mod_workshopplus', 'submission_attachment',
    ...
    protected function helper_sortable_heading($text, $sortid=null, $sortby=null, $sorthow=null) {
        ...
        if (!is_null($sortid)) {
            if ($sortby !== $sortid or $sorthow !== 'ASC') {
                ...
                $out .= $this->output->action_icon($url, new pix_icon('t/sort_asc', get_string('sortasc', 'workshopplus')),
                    null, array('class' => 'iconsort sort asc'));
            }
            if ($sortby !== $sortid or $sorthow !== 'DESC') {
                $url = new moodle_url($PAGE->url);
                $url->params(array('sortby' => $sortid, 'sorthow' => 'DESC'));
                $out .= $this->output->action_icon($url, new pix_icon('t/sort_desc', get_string('sortdesc', 'workshopplus')),
    ...
    protected function helper_grading_report_submission(stdclass $participant) {
        ...
        if (is_null($participant->submissionid)) {
            $out = $this->output->container(get_string('nosubmissionfound', 'workshopplus'), 'info');
        } else {
            $url = new moodle_url('/mod/workshopplus/submission.php',
    ...
    protected function helper_grading_report_assessment($assessment, $shownames, array $userinfo, $separator) {
        ...
        if (is_null($assessment)) {
            return get_string('nullgrade', 'workshopplus');
        }
        $a = new stdclass();
        $a->grade = is_null($assessment->grade) ? get_string('nullgrade', 'workshopplus') : $assessment->grade;
        $a->gradinggrade = is_null($assessment->gradinggrade) ? get_string('nullgrade', 'workshopplus') : $assessment->gradinggrade;
        ...
        if (is_null($assessment->gradinggradeover)) {
            if ($a->weight == 1) {
                $grade = get_string('formatpeergrade', 'workshopplus', $a);
            } else {
                $grade = get_string('formatpeergradeweighted', 'workshopplus', $a);
            }
        } else {
            $a->gradinggradeover = $assessment->gradinggradeover;
            if ($a->weight == 1) {
                $grade = get_string('formatpeergradeover', 'workshopplus', $a);
            } else {
                $grade = get_string('formatpeergradeoverweighted', 'workshopplus', $a);
            }
        }
        $url = new moodle_url('/mod/workshopplus/assessment.php',
    ...
    protected function helper_grading_report_grade($grade, $over=null) {
        $a = new stdclass();
        $a->grade = is_null($grade) ? get_string('nullgrade', 'workshopplus') : $grade;
        if (is_null($over)) {
            $text = get_string('formataggregatedgrade', 'workshopplus', $a);
        } else {
            $a->over = is_null($over) ? get_string('nullgrade', 'workshopplus') : $over;
            $text = get_string('formataggregatedgradeover', 'workshopplus', $a);
        }
        return $text;
    }
