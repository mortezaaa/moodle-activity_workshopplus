...
if ($id) { // submission is specified
	...
	if ($grouphassubmitted){
		$submission = $groupmatesubmission;
	}
    elseif (!$submission = $workshopplus->get_submission_by_author($USER->id)) {
        ...
        $owngroupsubmission = true; // Set $owngroupsubmission if the author belongs to the current user's group
    }
}
